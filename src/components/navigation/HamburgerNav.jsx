import React, { useState } from "react";

const HamburgerNav = ({ toggleEachDropdown, isMenuOpen }) => {
  return (
    <div className="relative md:w-full md:mt-10 md:flex-grow md:h-full">
      <img
        src={isMenuOpen ? "/img/close.svg" : "/img/hamburger.svg"}
        alt=""
        className="size-10 md:hidden gentleZoom"
        onClick={toggleEachDropdown}
        id="hamburger"
      />

      {/* Menu Links */}
      <div
        className={`absolute top-11 -right-4 bg-secondary text-white md:text-black w-screen h-96 flex flex-col items-center justify-evenly transition-all duration-300 ease-in-out transform md:w-full md:static md:flex md:justify-evenly md:space-y-6 md:pl-4 md:h-full md:bg-white
      ${
        isMenuOpen ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"
      } md:scale-y-100 md:opacity-100 origin-top`}
      >
        {/* Home */}
        <a
          href="#home"
          className="navLink"
          onClick={() => toggleEachDropdown({ target: { id: "hamburger" } })}
        >
          <p>Home</p>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
            stroke="currentColor"
            className="size-6 inline-block ml-2"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"
            />
          </svg>
        </a>
        {/* Skills */}
        <a
          href="#skills"
          className="navLink "
          onClick={() => toggleEachDropdown({ target: { id: "hamburger" } })}
        >
          <p>Skills</p>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
            stroke="currentColor"
            className="size-6 ml-2"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z"
            />
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
            />
          </svg>
        </a>
        {/* Projects */}
        <a
          href="#projects"
          className="navLink"
          onClick={() => toggleEachDropdown({ target: { id: "hamburger" } })}
        >
          <p>Projects</p>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
            stroke="currentColor"
            className="size-6 inline-block ml-2"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M14.25 9.75 16.5 12l-2.25 2.25m-4.5 0L7.5 12l2.25-2.25M6 20.25h12A2.25 2.25 0 0 0 20.25 18V6A2.25 2.25 0 0 0 18 3.75H6A2.25 2.25 0 0 0 3.75 6v12A2.25 2.25 0 0 0 6 20.25Z"
            />
          </svg>
        </a>
        {/* Contact */}
        <a
          href="#contact"
          className="navLink"
          onClick={() => toggleEachDropdown({ target: { id: "hamburger" } })}
        >
          <p>Contact</p>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
            stroke="currentColor"
            className="size-6 inline-block ml-2"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"
            />
          </svg>
        </a>
        {/* Social icons */}
        <div className="flex justify-center items-center pl-2">
          <a
            href="https://www.linkedin.com/in/jon-leyshon-338534a9/"
            target="_blank"
            className="pr-3 gentleZoom"
          >
            <img
              src="/img/linkedin.svg"
              alt="Jon Leyshon's LinkedIn"
              className="w-10"
            />
          </a>
          <a
            href="https://github.com/JonLeyshon"
            target="_blank"
            className="gentleZoom"
          >
            <img
              src="/img/github.svg"
              alt="Jon Leyshon's github"
              className="w-10"
            />
          </a>
        </div>
      </div>
    </div>
  );
};

export default HamburgerNav;
